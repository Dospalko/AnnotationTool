...
function PDFTokenViewer(props) {
  ...
  const assignAnnotationToToken = useCallback(
    (tokenId, annotationId) => {
      ...
      handleSaveTokens(); // Ensure this function is called with updated tokens
    },
    [tokens, annotations, handleSaveTokens]
  ); // Add handleSaveTokens as a dependency

  const removeAnnotationFromToken = useCallback(
    (tokenId) => {
      const updatedTokens = tokens.map((token) => {
        if (token.id === tokenId) {
    ...
    () =>
      tokens.map((token, index) => (
        <div key={`${token.word}-${index}`} className="relative flex space-x-2">
          <span
            className={`font-semibold cursor-pointer p-1 rounded ${
              token.annotation ? "text-white" : "text-gray-800"
            }`}
  ...
  return (
    <div className="font-base">
      ...
      <div className="border-t border-black p-2" id="tokenContainer">
        <h1 className="text-2xl m-auto font-bold flex justify-center items-center bg-black w-max text-white  p-2 text-center my-4">
          TOKENIZED
        </h1>
        <div className="flex justify-center m-auto items-center text-center mb-10">
          ...
          <button
            ...
            className="my-8 ml-4 gap-10 bg-green-500 text-white p-2 rounded hover:bg-green-600 transition duration-300"
          >
            Export Annotations
          </button>
        </div>
